...
const ParentView = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    ...
    // Then poll every 10 seconds for real-time updates (more aggressive than before)
    const interval = setInterval(() => {
      loadFamilyHeartbeats();
      
      // Debug logging to see what's happening
      if (familyHeartbeats.length > 0) {
        familyHeartbeats.forEach(hb => {
          const status = checkHeartbeatStatus(hb.lastHeartbeat);
          const timeSince = getTimeSinceHeartbeat(hb.lastHeartbeat);
          console.log(`Barrier Monitor - ${hb.childName}: ${status} (${timeSince})`);
        });
      }
    }, 10000);
  ...
  const loadFamilyHeartbeats = async () => {
    try {
      ...
      setFamilyHeartbeats(heartbeats);
    } catch (error) {
