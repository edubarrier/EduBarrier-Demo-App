...
import { Book, Users, Trophy, XCircle, Plus, LogOut, Eye, EyeOff, Trash2, Upload, Shield } from 'lucide-react';
import { storage, db, supabase, ensureUserInDatabase, fileUpload } from './supabase';

// MODULE-LEVEL CACHE: Stores optimistic states that persist across component re-renders/remounts
// Key = childId, Value = { value: any, timestamp: number }
const optimisticLockCache = {};
const optimisticExtraCreditCache = {};

// Helper to set optimistic lock state
const setOptimisticLock = (childId, locked) => {
  optimisticLockCache[childId] = { locked, timestamp: Date.now() };
};

// Helper to get optimistic lock state (returns null if expired or not set)
const getOptimisticLock = (childId) => {
  const cached = optimisticLockCache[childId];
  if (!cached) return null;
  // Expire after 10 seconds (longer than any re-render trigger)
  if (Date.now() - cached.timestamp > 10000) {
    delete optimisticLockCache[childId];
    return null;
  }
  return cached.locked;
};

// Helper to clear optimistic lock state
const clearOptimisticLock = (childId) => {
  delete optimisticLockCache[childId];
};

// Helper to set optimistic extra credit state
const setOptimisticExtraCredit = (childId, enabled) => {
  optimisticExtraCreditCache[childId] = { enabled, timestamp: Date.now() };
};

// Helper to get optimistic extra credit state (returns null if expired or not set)
const getOptimisticExtraCredit = (childId) => {
  const cached = optimisticExtraCreditCache[childId];
  if (!cached) return null;
  if (Date.now() - cached.timestamp > 10000) {
    delete optimisticExtraCreditCache[childId];
    return null;
  }
  return cached.enabled;
};

// Helper to clear optimistic extra credit state
const clearOptimisticExtraCredit = (childId) => {
  delete optimisticExtraCreditCache[childId];
};
...
const App = () => {
  // Ref to prevent duplicate email confirmation processing
  const emailConfirmationProcessed = useRef(false);
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      // Note: We don't call ensureUserInDatabase here because it's handled in the login flow
      // This prevents duplicate family creation when the SIGNED_IN event fires
    });
  ...
  const init = async () => {
    // ===== NEW: Check for hash fragment authentication (from email confirmation) =====
    const hash = window.location.hash;
    if (hash && hash.includes('access_token')) {
      // Prevent duplicate processing with ref
      if (emailConfirmationProcessed.current) {
        console.log('Already processing email confirmation, skipping...');
        return;
      }
      
      // Mark as processed IMMEDIATELY
      emailConfirmationProcessed.current = true;
      
      // Clear hash IMMEDIATELY to prevent re-triggering
      window.history.replaceState(null, '', window.location.pathname);
      
      console.log('\u{1F527} Email confirmation detected in URL hash');
      
      try {
        // Just verify the session exists (Supabase has already confirmed the email)
        const { data, error } = await supabase.auth.getSession();
        
        if (data?.session?.user) {
          console.log('\u{2705} Email confirmed! User:', data.session.user.email);
          
          // Show success message and prompt to login
          setSuccessMessage('\u{2705} Email confirmed! Please log in with your credentials.');
          ...
          // No session means email was already confirmed or link expired
          setSuccessMessage('\u{2705} Email confirmed! Please log in with your credentials.');
          setLoading(false);
        }
      } catch (err) {
      ...
      return; // Exit early, don't continue with normal init
    }
    // ===== END NEW CODE =====
...
const ParentView = ({ user, onLogout }) => {
  ...
  // Barrier system state
  const [barrierStatus, setBarrierStatus] = useState(null);
  const [familyHeartbeats, setFamilyHeartbeats] = useState([]);
  const [barrierMonitoring, setBarrierMonitoring] = useState(false);
  const [lastCheckTime, setLastCheckTime] = useState(null); // Track when we last checked for heartbeats
  ...
  // Barrier system: Load barrier status on mount
  useEffect(() => {
    loadBarrierStatus();
  }, [user.family_id || user.familyId]);

  // Barrier system: Monitor heartbeats when barrier is active
  useEffect(() => {
    console.log('Barrier monitoring useEffect triggered. barrierMonitoring =', barrierMonitoring);
    
    if (!barrierMonitoring) {
      console.log('Barrier monitoring is OFF - not starting interval');
      return;
    }
    
    console.log('Barrier monitoring is ON - starting 30-second interval');
    
    // Load heartbeats immediately
    const loadHeartbeats = async () => {
      try {
        console.log('Loading family heartbeats...');
        const familyId = user.family_id || user.familyId;
        const heartbeats = await db.getFamilyHeartbeats(familyId);
        console.log('Received heartbeats:', heartbeats);
        
        // Only update state if data actually changed
        setFamilyHeartbeats(prev => {
          const hasChanged = JSON.stringify(prev) !== JSON.stringify(heartbeats);
          if (hasChanged) {
            console.log('Heartbeats changed - updating state');
            // Only update last check time when data actually changed
            setLastCheckTime(new Date());
            return heartbeats;
          }
          console.log('Heartbeats unchanged - skipping update');
          return prev;
        });
      } catch (error) {
        console.error('Error loading family heartbeats:', error);
      }
    };
    
    loadHeartbeats(); // Load immediately
    
    // Then poll every 30 seconds
    const interval = setInterval(() => {
      console.log('30-second interval fired - loading heartbeats');
      loadHeartbeats();
    }, 30000);
    
    return () => {
      console.log('Cleaning up barrier monitoring interval');
      clearInterval(interval);
    };
  }, [barrierMonitoring, user.family_id, user.familyId]);
  ...
  const deleteAssignment = async (assignmentId) => {
    ...
    try {
      ...
      console.log('\u{2713} Database deletion returned true');
      ...
      console.error('\u{2717} Error deleting assignment:', error);
  ...
  const loadBarrierStatus = async () => {
    try {
      const familyId = user.family_id || user.familyId;
      const status = await db.getBarrierStatus(familyId);
      setBarrierStatus(status);
      
      // If barrier is active, start monitoring
      if (status && status.is_active) {
        setBarrierMonitoring(true);
      }
    } catch (error) {
  ...
  const toggleBarrier = async () => {
    try {
      const familyId = user.family_id || user.familyId;
      const currentStatus = barrierStatus?.is_active || false;
      const newStatus = !currentStatus;
      
      // Toggle barrier in database
      const updated = await db.toggleBarrier(familyId, newStatus, user.id);
      setBarrierStatus(updated);
      
      // Update monitoring state
      setBarrierMonitoring(newStatus);
      
      if (newStatus) {
        setMessage('Barrier activated - monitoring started');
      } else {
        setMessage('Barrier deactivated - monitoring stopped');
      }
      setTimeout(() => setMessage(''), 3000);
    } catch (error) {
      console.error('Error toggling barrier:', error);
      setMessage('Error toggling barrier. Please try again.');
      setTimeout(() => setMessage(''), 3000);
    }
  };

  const loadFamilyHeartbeats = async () => {
    try {
      const familyId = user.family_id || user.familyId;
      const heartbeats = await db.getFamilyHeartbeats(familyId);
      
      // Only update state if data actually changed (prevents unnecessary re-renders and flickering)
      setFamilyHeartbeats(prev => {
        // Compare the data to see if anything changed
        const hasChanged = JSON.stringify(prev) !== JSON.stringify(heartbeats);
        if (hasChanged) {
          return heartbeats;
        }
  ...
  const checkHeartbeatStatus = (lastHeartbeat) => {
    ...
    // Consider offline if no heartbeat for 60 seconds (2 missed 30-second cycles)
    if (secondsAgo > 60) return 'offline';
    return 'online';
  };
  ...
  const getTimeSinceLastCheck = () => {
    if (!lastCheckTime) return 'Not checked yet';
    
    const now = new Date();
    const secondsAgo = Math.floor((now - lastCheckTime) / 1000);
  ...
  const toggleChildLock = async (childId) => {
    ...
    try {
      ...
      console.log('===== TOGGLE LOCK END ===== Call ID:', callId);
      ...
      console.error('\u{2717} Error toggling lock:', error);
  ...
  const clearChildTime = async (childId) => {
    ...
    // Don't reload kids - let the optimistic update in ChildCard handle the UI
    // This prevents flickering from re-rendering all cards
  };
  ...
  // Barrier Status Bar Component - Shows on all parent pages
  const BarrierStatusBar = ({ barrierStatus, familyHeartbeats, checkHeartbeatStatus }) => {
    if (!barrierStatus) return null;
    
    const isActive = barrierStatus.is_active;
    const hasAlert = isActive && familyHeartbeats.some(hb => checkHeartbeatStatus(hb.lastHeartbeat) === 'offline');
    
    let borderColor, bgColor, textColor, statusText, statusLabel;
    
    if (!isActive) {
      borderColor = 'border-gray-300';
      bgColor = 'bg-white';
      textColor = 'text-gray-600';
      statusLabel = 'BARRIER STATUS:';
    ...
    return (
      <div className={`mx-4 mt-4 mb-2 px-6 py-4 border-2 ${borderColor} ${bgColor} rounded-2xl flex items-center gap-3 ${hasAlert ? 'animate-pulse' : ''}`}>
        <span className="text-3xl">{'\u{1F514}'}</span>
        <div className="flex items-baseline gap-2">
          <span className={`text-xs font-bold ${textColor} uppercase tracking-wide`}>{statusLabel}</span>
          <span className={`text-lg font-bold ${textColor === 'text-gray-600' ? 'text-gray-800' : textColor}`}>{statusText}</span>
        </div>
      </div>
    );
  };

  // Child Heartbeat Card Component - Shows child's monitoring status
  const ChildHeartbeatCard = ({ child, heartbeat, checkHeartbeatStatus, getTimeSinceHeartbeat, getTimeSinceLastCheck, barrierActive }) => {
    const status = checkHeartbeatStatus(heartbeat?.lastHeartbeat);
    const timeSince = getTimeSinceHeartbeat(heartbeat?.lastHeartbeat);
    const [, forceUpdate] = useState(0);
    
    // Force component to re-render every second to update "Last Check" timestamp
    useEffect(() => {
        ...
        <div className="bg-white rounded-xl p-5 border-2 border-gray-200">
          <div className="flex items-start justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-gray-600">Not Monitored</span>
            </div>
          </div>
          <div className="text-center py-6 text-gray-500">
            <Users className="mx-auto text-gray-400 mb-2" size={32} />
            <p className="mt-2 text-sm">Activate barrier to start monitoring</p>
          </div>
        </div>
      );
    }
    ...
    const neverConnected = status === 'never';
    
    const borderColor = isOnline ? 'border-green-500' : isOffline ? 'border-red-500' : 'border-gray-300';
    const bgColor = isOnline ? 'bg-gradient-to-br from-green-50 to-emerald-50' : isOffline ? 'bg-gradient-to-br from-red-50 to-pink-50' : 'bg-white';
    ...
    return (
      <div className={`${bgColor} rounded-xl p-5 border-2 ${borderColor}`}>
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <span className={`w-3 h-3 rounded-full ${isOnline ? 'bg-green-500 animate-pulse' : isOffline ? 'bg-red-500' : 'bg-gray-400'}`}></span>
            <span className={`text-sm font-medium ${isOnline ? 'text-green-700' : isOffline ? 'text-red-700' : 'text-gray-600'}`}>
              {isOnline ? 'App Running' : isOffline ? 'App Not Running' : 'Waiting...'}
            </span>
          </div>
        </div>
        ...
        {isOnline && (
          <div className="bg-white/50 rounded-lg p-4 mb-3">
            <div className="flex items-center gap-2 mb-2">
              <Shield className="text-green-600 inline" size={20} />
              <span className="text-sm font-semibold text-green-700">Connected & Responding</span>
            </div>
            <p className="text-xs text-gray-600">Last heartbeat: {timeSince}</p>
          </div>
        )}
        
        {isOffline && (
          <div className="bg-red-100 border-2 border-red-200 rounded-lg p-4 mb-3">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-lg">{'\u{1F6A8}'}</span>
              <span className="text-sm font-bold text-red-700">App Closed or Force-Quit</span>
            </div>
            <p className="text-xs text-red-600 mb-3">
              {child.name}'s app stopped responding. The app was likely force-quit or the device was turned off.
            </p>
            <div className="flex gap-2">
              <button className="px-3 py-1 bg-red-600 text-white text-xs rounded-lg hover:bg-red-700">
                Send Message
              </button>
              <button className="px-3 py-1 bg-white text-red-600 text-xs rounded-lg border border-red-600 hover:bg-red-50">
                View Details
              </button>
            </div>
          </div>
        ...
        {neverConnected && (
          <div className="text-center py-4 text-gray-500">
            ...
            <p className="mt-2 text-xs">Waiting for first heartbeat...</p>
          </div>
        )}
        
        {/* Taller monitor box with heart rate animation */}
        <div className="bg-gray-900 text-green-400 rounded-lg p-4 font-mono text-xs min-h-[200px]">
          <div className="font-bold mb-3 text-white">{'\u{1F4E1}'} HEARTBEAT MONITOR</div>
          <div className="border-t border-gray-700 pt-3 space-y-2">
            <div className="flex justify-between">
              <span>Status:</span>
              <span className={isOnline ? 'text-green-400' : isOffline ? 'text-red-400' : 'text-gray-400'}>
                {isOnline ? 'âœ— ALIVE' : isOffline ? 'âœ“ NO RESPONSE' : 'â³ WAITING'}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Last Heartbeat:</span>
              <span className="text-gray-300">{timeSince}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="mt-4 pt-3 border-t border-gray-700">
              <div className="relative h-16 bg-gray-950 rounded overflow-hidden">
                {/* Grid lines (like hospital monitor) */}
                <div className="absolute inset-0 opacity-20">
                  {[...Array(8)].map((_, i) => (
                    <div key={i} className="absolute w-full border-t border-gray-600" style={{ top: `${(i + 1) * 12.5}%` }}></div>
                  ))}
                  {[...Array(12)].map((_, i) => (
                    <div key={i} className="absolute h-full border-l border-gray-600" style={{ left: `${(i + 1) * 8.33}%` }}></div>
                  ))}
                </div>
                ...
                <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
                  {isOnline ? (
                    // Animated heartbeat (alive)
                    <polyline
                      points="0,32 50,32 55,10 60,50 65,20 70,32 120,32 125,10 130,50 135,20 140,32 190,32 195,10 200,50 205,20 210,32 260,32 265,10 270,50 275,20 280,32 330,32 335,10 340,50 345,20 350,32 400,32"
                      fill="none"
                      stroke="#10b981"
                      strokeWidth="2"
              ...
              <div className={`text-center mt-2 text-xs ${isOnline ? 'text-green-400' : 'text-red-400'}`}>
                {isOnline ? 'âœ… Active Signal âœ…' : 'âœ… No Signal âœ…'}
              </div>
            </div>
          </div>
        </div>
  ...
  // Separate component for child card to properly use hooks - Memoized to prevent flickering
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    // Use ref for processing guard - refs update synchronously to prevent double-clicks
    const isProcessingRef = useRef(false);
    const [isProcessingUI, setIsProcessingUI] = useState(false); // For visual feedback only
    
    // Track if user has made optimistic updates - these take precedence over database
    const hasOptimisticTimeUpdate = useRef(false);
    const hasLoadedRef = useRef(false);
    
    // COMPUTED DISPLAY VALUES: Check module-level cache first (survives component remount)
    // Then fall back to component state
    const cachedLock = getOptimisticLock(child.id);
    const displayLocked = cachedLock !== null ? cachedLock : isLocked;
    
    const cachedExtraCredit = getOptimisticExtraCredit(child.id);
    const displayExtraCredit = cachedExtraCredit !== null ? cachedExtraCredit : extraCreditEnabled;
    
    useEffect(() => {
      // Only load ONCE on first mount - never reload to preserve optimistic updates
      ...
      const checkStatusAndTime = async () => {
        ...
        // Only update lock state if NO optimistic update is cached
        if (getOptimisticLock(child.id) === null) {
          setIsLocked(newLockStatus);
        }
        if (!hasOptimisticTimeUpdate.current) {
          setActualTimeEarned(newTimeEarned);
        }
        // Only update extra credit state if NO optimistic update is cached
        if (getOptimisticExtraCredit(child.id) === null) {
          setExtraCreditEnabled(newExtraCreditStatus);
        }
      };
      
      checkStatusAndTime();
      
      // NO POLLING - only check once on mount to prevent flickering
      // Updates will happen when user clicks buttons (toggle lock, clear time, etc.)
    }, []); // Empty dependency array - truly only once
        ...
        {isEditing ? (
          ...
          <>
            ...
            <div className="space-y-2">
              {/* Lock Status Banner */}
              {displayLocked && actualTimeEarned === 0 && (
                <div className="bg-orange-100 border-2 border-orange-300 rounded-lg p-2 text-center">
                  <p className="text-orange-800 font-bold text-sm">{'\u{1F512}'} Auto-Locked (No Time)</p>
              ...
              {displayLocked && actualTimeEarned > 0 && (
                <div className="bg-red-100 border-2 border-red-300 rounded-lg p-2 text-center">
                  <p className="text-red-700 font-bold text-sm">{'\u{1F512}'} Manually Locked</p>
                </div>
              )}
              ...
              <button
                onClick={async () => {
                  ...
                  // Use ref for synchronous check - prevents race condition with double-clicks
                  if (isProcessingRef.current) {
                    console.log('BLOCKED by isProcessingRef');
                    return;
                  }
                  isProcessingRef.current = true;
                  console.log('Processing started, ref set to true');
                  setIsProcessingUI(true); // Visual feedback
                  try {
                    // Calculate the new lock state
                    const newLockState = !displayLocked;
                    console.log('Setting intended lock state to:', newLockState);
                    // Store in MODULE-LEVEL cache (survives component remount!)
                    setOptimisticLock(child.id, newLockState);
                    // Also update component state
                    setIsLocked(newLockState);
                    // Then update database in background
                    await toggleChildLock(child.id);
                  } finally {
                    // Always re-enable button after 1 second
                    setTimeout(() => {
                      isProcessingRef.current = false;
                      setIsProcessingUI(false);
                    }, 1000);
                    // Clear the cache after 10 seconds to allow fresh data
                    setTimeout(() => {
                      clearOptimisticLock(child.id);
                    }, 10000);
                  }
                }}
                className={`w-full py-2 rounded-lg text-sm font-bold ${isProcessingUI ? 'opacity-50 cursor-not-allowed' : ''} ${
                  displayLocked 
                    ? 'bg-green-500 text-white hover:bg-green-600' 
                    : 'bg-red-500 text-white hover:bg-red-600'
                }`}
              >
                {displayLocked ? 'Unlock Screen Access' : 'Lock Screen Access'}
              </button>
              
              {/* Clear Time Button */}
              <button
                onClick={async () => {
                  if (isProcessingRef.current) return; // Synchronous check
                  if (!confirm('Are you sure you want to clear all screen time?')) return;
                  isProcessingRef.current = true;
                  setIsProcessingUI(true);
                  try {
                    // Mark that user has made optimistic updates
                    hasOptimisticTimeUpdate.current = true;
                    hasOptimisticLockUpdate.current = true;
                    // Optimistic update: set to 0 and locked immediately
                    setActualTimeEarned(0);
                    setIsLocked(true);
                    // Then update database in background
                    await clearChildTime(child.id);
                  } finally {
                    setTimeout(() => {
                      isProcessingRef.current = false;
                      setIsProcessingUI(false);
                ...
                className={`w-full py-2 rounded-lg text-sm font-bold ${isProcessingUI ? "opacity-50 cursor-not-allowed" : ""} bg-orange-500 text-white hover:bg-orange-600 transition-all`}
              >
                Clear Screen Time
              </button>
              ...
              <button
                onClick={async () => {
                  ...
                  setIsProcessingUI(true);
                  try {
                    // Calculate the new extra credit state
                    const newExtraCreditState = !displayExtraCredit;
                    // Store in MODULE-LEVEL cache (survives component remount!)
                    setOptimisticExtraCredit(child.id, newExtraCreditState);
                    // Also update component state
                    setExtraCreditEnabled(newExtraCreditState);
                    // Then update database in background
                    await toggleExtraCredit(child.id);
                  } finally {
                    setTimeout(() => {
                      isProcessingRef.current = false;
                      setIsProcessingUI(false);
                    ...
                    // Clear the cache after 10 seconds to allow fresh data
                    setTimeout(() => {
                      clearOptimisticExtraCredit(child.id);
                    }, 10000);
                  }
                }}
              ...
              >
                {displayExtraCredit ? '\u{2B50} Extra Credit: ON' : '\u{2B50} Extra Credit: OFF'}
              </button>
    ...
    // Custom comparison: only re-render if data for THIS specific child changed
    // Return TRUE if props are equal (skip re-render)
    // Return FALSE if props changed (do re-render)
    
    // Check if it's the same child
    if (prevProps.child.id !== nextProps.child.id) return false;
    
    // Check if this child's assignments changed
    const prevChildAssigns = prevProps.assigns.filter(a => a.childId === prevProps.child.id || a.child_id === prevProps.child.id);
    const nextChildAssigns = nextProps.assigns.filter(a => a.childId === nextProps.child.id || a.child_id === nextProps.child.id);
    if (JSON.stringify(prevChildAssigns) !== JSON.stringify(nextChildAssigns)) return false;
    
    // Check if editing this specific child
    const prevEditing = prevProps.editingChild?.id === prevProps.child.id;
    const nextEditing = nextProps.editingChild?.id === nextProps.child.id;
    if (prevEditing !== nextEditing) return false;
    
    // If we got here, nothing relevant to THIS child changed
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50 pb-20">
      ...
      {/* Barrier Status Bar - Shows on all parent pages */}
      <BarrierStatusBar 
        barrierStatus={barrierStatus}
        familyHeartbeats={familyHeartbeats}
        checkHeartbeatStatus={checkHeartbeatStatus}
      />
      ...
      <div className="max-w-6xl mx-auto p-4">
                    ...
                    <div key={course.id} className="border-2 border-gray-200 rounded-2xl p-6 hover:border-teal-300 hover:shadow-lg transition-all">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4 text-xs">
                        <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full">
                          {'\u{1F4CA}'} {(course.difficulty || 'beginner').charAt(0).toUpperCase() + (course.difficulty || 'beginner').slice(1)}
                        </span>
                        <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-full">
                          {'\u{23F1}'} {course.estimatedTime || course.estimated_time || 30} min
                        </span>
                        <span className="bg-pink-100 text-pink-700 px-3 py-1 rounded-full">
                          {'\u{1F476}'} Ages {course.ageRangeMin || course.age_range_min || 6}-{course.ageRangeMax || course.age_range_max || 16}
                        </span>
                      </div>
                                  ...
                                  <div key={a.id} className="bg-white rounded-lg p-4 flex items-center justify-between">
                                    <div className="flex-1">
                                      ...
                                      <p className="text-sm text-gray-600">
                                        {a.mins} min reward {'\u{2022}'} {a.passScore || a.pass_score}% to pass
                                      </p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                                        ...
                                        {a.status === 'completed' ? `${String.fromCodePoint(0x2713)} ${a.score}%` : 'Pending'}
                                      </span>
                                      {a.status === 'pending' && (
                                        <button
                                          onClick={() => deleteAssignment(a.id)}
                      ...
                      return (
                        <div key={a.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {child?.name} {'\u{2022}'} {a.mins}min reward
                              {isFuture && (
                                <span className="ml-2 text-orange-600 font-semibold">
                                  {'\u{1F4C5}'} {new Date(availDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                                </span>
                              )}
                            </p>
                          </div>
        ...
        {tab === 'barrier' && (
          <div className="space-y-4">
            <div className="bg-white rounded-3xl shadow-lg p-6">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">Barrier Monitoring</h2>
              
              {/* Toggle Control Section */}
              <div className="flex flex-col items-center mb-8">
                <div className="text-center mb-6">
                  <span className="text-8xl mb-2 block">{barrierStatus?.is_active ? '\u{1F6E1}' : '\u{1F514}'}</span>
                  <p className="text-xl font-bold text-gray-800 mb-1">
                    {barrierStatus?.is_active ? 'Monitoring Active' : 'Barrier Inactive'}
                  </p>
                  <p className="text-sm text-gray-600">
                    {barrierStatus?.is_active ? 'Checking every 30 seconds' : 'Tap below to start monitoring'}
                  </p>
                </div>
                ...
                <button
                  ...
                  className={`
                    ...
                    ${barrierStatus?.is_active 
                      ? 'bg-gradient-to-br from-green-400 to-emerald-600 text-white animate-pulse hover:from-green-500 hover:to-emerald-700' 
                      : 'bg-gradient-to-br from-gray-300 to-gray-500 text-white hover:from-gray-400 hover:to-gray-600'
                    }
                  `}
                >
                  <Shield size={48} className="mb-3" />
                  <span>{barrierStatus?.is_active ? 'Deactivate' : 'Activate'}</span>
                  <span>{barrierStatus?.is_active ? 'Barrier' : 'Barrier'}</span>
                </button>
              </div>
              ...
              <div className="border-t pt-6">
                ...
                {kids.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-lg font-medium">No children in household yet</p>
                    <p className="text-sm mt-2">Add children in the Manage tab to start monitoring</p>
                  </div>
                ) : (
                  <div className="grid md:grid-cols-2 gap-4">
                    {kids.map(child => {
                      const heartbeat = familyHeartbeats.find(hb => hb.childId === child.id);
                      return (
                        <ChildHeartbeatCard
                          key={child.id}
                          child={child}
                          ...
                          getTimeSinceLastCheck={getTimeSinceLastCheck}
                          barrierActive={barrierStatus?.is_active}
                        />
                      );
                    })}
                  </div>
              ...
              <div className="mt-6 bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
                <div className="flex gap-3">
                  <div className="text-2xl">{'\u{2139}'}</div>
                  <div>
                    <h4 className="font-bold text-blue-900 mb-2">How Barrier Works</h4>
                    <ul className="text-sm text-blue-800 space-y-1">
                      <li>• Click "Activate Barrier" to start monitoring</li>
                      <li>• System checks child's app every 30 seconds</li>
                      <li>• You'll be notified instantly if app is closed</li>
                      <li>• Child just needs to keep app open - no spam notifications</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="border-b pb-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500">(c) 2025 EduBarrier Inc. All rights reserved.</p>
                    <button 
                      onClick={() => {
...
const ChildView = ({ user, onLogout }) => {
  ...
  const [showingReview, setShowingReview] = useState(false);  // FIXED: Moved to top level to prevent render loop
  ...
  useEffect(() => {
    ...
    const checkLockStatus = async () => {
      ...
      setIsLocked(prev => {
        if (prev !== lockStatus) {
          console.log('Child lock status changed:', prev, '\u{2192}', lockStatus);
          return lockStatus;
        }
  ...
  // Barrier system: Send heartbeat every 30 seconds
  useEffect(() => {
    const sendHeartbeat = async () => {
      try {
        const familyId = user.family_id || user.familyId;
        await db.sendHeartbeat(user.id, familyId);
  ...
  }, [user.id, user.family_id || user.familyId]);
  ...
  const calcResults = async (ans, course) => {
    ...
    if (updated) {
      console.log('\u{2713} Database update successful!', updated);
    } else {
      console.error('\u{2717} Database update failed - but continuing with state update');
    }
  ...
  const closeAll = async () => {
    ...
    setViewing(false);
    setResults(null);
    setShowingReview(false);  // FIXED: Reset review state
    // FIXED: Reload assignments from database to ensure UI shows updated status
    console.log('Closing quiz - reloading assignments from database');
    await loadData();
  };
                      ...
                      return (
                        <div key={practiceIdx} className="bg-purple-50 rounded-xl p-4 border-2 border-purple-200">
                          ...
                          {isCorrect !== null && (
                            ...
                            >
                              {isCorrect ? '\u{2713} Correct! Great job!' : '\u{2717} Not quite. Try again!'}
                            </div>
                          )}
                          ...
                          <div className="flex gap-2">
                            {practice.hint && (
                              ...
                              >
                                {showHint ? '\u{25BC} Hide Hint' : '\u{25B6} Show Hint'}
                              </button>
                            )}
                            <button
                        ...
                        <button
                          ...
                          className={'w-full text-left px-4 py-3 rounded-xl transition-all font-semibold ' + 
                            (answers[idx] === value 
                              ...
                              : 'bg-white border-2 hover:border-purple-400')}
                        >
                          {value ? '\u{2713} True' : '\u{2717} False'}
                        </button>
                      ))}
                    </div>
                  )}
                    ...
                    <div className="space-y-3">
                      ...
                      {question.pairs.map((pair, pairIdx) => (
                        <div key={pairIdx} className="flex items-center gap-3 bg-white p-3 rounded-xl">
                          ...
                          <div className="text-gray-400">{'\u{2192}'}</div>
                          <select
...
if (results) {
    // FIXED: Removed useState from conditional - now using top-level state
                  ...
                  <div key={idx} className={`p-6 rounded-2xl border-2 ${isCorrect ? 'bg-green-50 border-green-300' : 'bg-red-50 border-red-300'}`}>
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className={`px-3 py-1 rounded-full text-sm font-bold ${isCorrect ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}`}>
                        {isCorrect ? '\u{2713} Correct' : '\u{2717} Wrong'}
                      </span>
                    </div>
                          ...
                          <div className="space-y-1">
                            {question.pairs.map((pair, pairIdx) => (
                              <p key={pairIdx} className={isCorrect ? 'text-green-700' : 'text-red-700'}>
                                {pair.left} {'\u{2192}'} {question.pairs[userAnswer?.[pairIdx]]?.right || '(Not matched)'}
                              </p>
                            ))}
                          </div>
                        )}
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50">
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className={`px-4 py-2 rounded-lg font-semibold text-sm ${
              ...
              {isLocked ? '\u{1F512} Locked' : '\u{2713} Unlocked'}
            </div>
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-6">
          ...
          {isLocked && remaining === 0 && (
            <div className="bg-orange-100 border-2 border-orange-300 rounded-xl p-4 mb-4">
              <div className="flex items-center gap-3">
                <div className="text-3xl text-orange-600 font-bold">{'\u{23F8}'}</div>
                <div className="flex-1">
          ...
          {isLocked && remaining > 0 && (
            <div className="bg-red-100 border-2 border-red-300 rounded-xl p-4 mb-4">
              <div className="flex items-center gap-3">
                <div className="text-3xl text-red-600 font-bold">{'\u{1F512}'}</div>
                <div className="flex-1">
          ...
          <p className="text-center text-sm text-gray-500 mt-2">
            {timeData.used}m used {'\u{2022}'} {timeData.total}m earned
          </p>
          
          <div className="mt-4 bg-purple-50 border border-purple-200 rounded-xl p-3">
            <p className="text-xs text-purple-800 text-center">
              {'\u{1F4A1}'} Tip: Pause your timer when stepping away to save your screen time!
            </p>
          </div>
        </div>
        ...
        {assigns.filter(a => a.status === 'completed').length > 0 && (
          <div className="bg-white rounded-3xl shadow-lg p-6">
            <h2 className="text-xl font-bold mb-4 text-green-600">{'\u{2713}'} Completed Courses</h2>
            <div className="space-y-3">
                ...
                return (
                  <div key={a.id} className="border-2 border-green-200 bg-green-50 rounded-2xl p-4">
                    ...
                    <div className="flex gap-3 mt-2 text-sm text-gray-600">
                      <span className="bg-green-100 px-3 py-1 rounded-full">{'\u{2713}'} Completed</span>
        ...
        {childTab === 'extraCredit' && (
          <>
            {!extraCreditEnabled ? (
              ...
              <div className="bg-white rounded-3xl shadow-lg p-6">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">{'\u{1F4DA}'} Course Library - Extra Credit</h2>
        ...
        {childTab === 'reportCard' && (
          <div className="bg-white rounded-3xl shadow-lg p-6">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">{'\u{1F4CA}'} My Report Card</h2>
...
const AdminView = ({ user, onLogout }) => {
  ...
  const loadData = async () => {
    try {
      ...
      // Load all families directly from families table
      const famList = await db.getAllFamilies() || [];
      setFamilies(famList);
    } catch (err) {
  ...
  const checkSystemStatus = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        newStatus.storage = { 
          status: 'online', 
          message: `${data.length} bucket(s) ${String.fromCodePoint(0x2022)} ${responseTime}ms` 
        };
      }
    } catch (err) {
    ...
    try {
      ...
      if (!courses) {
        ...
        newStatus.courses = { 
          status: 'warning', 
          message: `${courses.length} courses ${String.fromCodePoint(0x2022)} Slow (${responseTime}ms)` 
        };
      } else {
        console.log('Courses OK:', courses.length, 'courses in', responseTime, 'ms');
        newStatus.courses = { 
          status: 'online', 
          message: `${courses.length} courses ${String.fromCodePoint(0x2022)} ${responseTime}ms` 
        };
      }
    } catch (err) {
    ...
    try {
      ...
      if (!users || !authData.session) {
        ...
        newStatus.users = { 
          status: 'warning', 
          message: `${users.length} users ${String.fromCodePoint(0x2022)} Slow (${responseTime}ms)` 
        };
      } else {
        console.log('Users OK:', users.length, 'users in', responseTime, 'ms');
        newStatus.users = { 
          status: 'online', 
          message: `${users.length} users ${String.fromCodePoint(0x2022)} ${responseTime}ms` 
        };
      }
    } catch (err) {
    ...
    // Final update
    setSystemStatus({ ...newStatus });
    console.log('\u{2705} System status check complete', newStatus);
  };
  ...
  const deleteUser = async (userId) => {
    ...
    try {
      ...
      if (deleteError) {
        console.error('\u{2717} Database delete failed:', deleteError);
      ...
      console.error('\u{2717} Error deleting user:', err);
              ...
              <div className="space-y-3">
                ...
                <div className={`flex justify-between items-center p-3 rounded-lg ${
                  ...
                  <span className={`font-semibold ${
                    ...
                    {systemStatus.storage.status === 'online' ? '\u{2713} Online' : 
                     systemStatus.storage.status === 'warning' ? '\u{26A0} Warning' : 
                     systemStatus.storage.status === 'offline' ? '\u{274C} Offline' : '\u{23F3} Checking'}
                  </span>
                </div>
                ...
                <div className={`flex justify-between items-center p-3 rounded-lg ${
                  ...
                  <span className={`font-semibold ${
                    ...
                    {systemStatus.courses.status === 'online' ? '\u{2713} Online' : 
                     systemStatus.courses.status === 'warning' ? '\u{26A0} Warning' : 
                     systemStatus.courses.status === 'offline' ? '\u{274C} Offline' : '\u{23F3} Checking'}
                  </span>
                </div>
                ...
                <div className={`flex justify-between items-center p-3 rounded-lg ${
                  ...
                  <span className={`font-semibold ${
                    ...
                    {systemStatus.users.status === 'online' ? '\u{2713} Online' : 
                     systemStatus.users.status === 'warning' ? '\u{26A0} Warning' : 
                     systemStatus.users.status === 'offline' ? '\u{274C} Offline' : '\u{23F3} Checking'}
                  </span>
                </div>
              </div>
            </div>
                            ...
                            {question.opts && question.opts.map((opt, optIdx) => (
                              <div key={optIdx} className={`flex items-center gap-2 p-2 rounded-lg transition-colors ${question.ans === optIdx ? 'bg-green-50 border-2 border-green-500' : 'border-2 border-gray-200'}`}>
                                ...
                                <span className={`text-sm font-bold ${question.ans === optIdx ? 'text-green-600' : 'text-gray-400'}`}>
                                  {question.ans === optIdx ? '\u{2713} CORRECT' : 'Wrong'}
                                </span>
                              </div>
                            ))}
                          </div>
                            ...
                            <div className="space-y-2 mb-3">
                              {(question.pairs || []).map((pair, pairIdx) => (
                                <div key={pairIdx} className="flex gap-2 items-center bg-white p-2 rounded-lg">
                                  ...
                                  />
                                  <span className="text-gray-400">{'\u{2192}'}</span>
                                  <input
                                    type="text"
                  ...
                  <div key={course.id} className="flex justify-between items-center p-4 border-2 rounded-xl">
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-600">
                        <span>{'\u{1F4DA}'} {course.sections?.length || 0} sections</span>
                        <span>{'\u{2014}'} {course.questions?.length || 0} questions</span>
                        <span>{'\u{23F1}'} {course.estimatedTime || course.estimated_time || 30} min</span>
                        <span className="capitalize">{'\u{1F4CA}'} {course.difficulty || 'beginner'}</span>
                        <span>{'\u{1F476}'} Ages {course.ageRangeMin || course.age_range_min || 6}-{course.ageRangeMax || course.age_range_max || 16}</span>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button 
                      ...
                      >
                        {loadingCourse ? (
                          <>
                            <span className="animate-spin">{'\u{23F8}'}</span>
                            Loading...
                          </>
                        ) : (
                          'Edit'
