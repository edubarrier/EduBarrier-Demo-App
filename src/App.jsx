                                  ...
                                  <div key={a.id} className="bg-white rounded-lg p-4 flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                                        ...
                                        {a.status === 'completed' ? `\u{2713} ${a.score}%` : 'Pending'}
                                      </span>
                                      {a.status === 'pending' && (
                                        <button
                                          onClick={() => deleteAssignment(a.id)}
  ...
  const checkSystemStatus = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        newStatus.storage = { 
          status: 'online', 
          message: `${data.length} bucket(s) \u{2022} ${responseTime}ms` 
        };
      }
    } catch (err) {
    ...
    try {
      ...
      if (!courses) {
        ...
        newStatus.courses = { 
          status: 'warning', 
          message: `${courses.length} courses \u{2022} Slow (${responseTime}ms)` 
        };
      } else {
        console.log('Courses OK:', courses.length, 'courses in', responseTime, 'ms');
        newStatus.courses = { 
          status: 'online', 
          message: `${courses.length} courses \u{2022} ${responseTime}ms` 
        };
      }
    } catch (err) {
    ...
    try {
      ...
      if (!users || !authData.session) {
        ...
        newStatus.users = { 
          status: 'warning', 
          message: `${users.length} users \u{2022} Slow (${responseTime}ms)` 
        };
      } else {
        console.log('Users OK:', users.length, 'users in', responseTime, 'ms');
        newStatus.users = { 
          status: 'online', 
          message: `${users.length} users \u{2022} ${responseTime}ms` 
        };
      }
    } catch (err) {
