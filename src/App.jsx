...
const ParentView = ({ user, onLogout }) => {
  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    // Sync state from refs on every render if optimistic update is active
    useEffect(() => {
      if (hasOptimisticLockUpdate.current && intendedLockState.current !== null) {
        if (isLocked !== intendedLockState.current) {
          console.log('Restoring optimistic lock state:', intendedLockState.current);
          setIsLocked(intendedLockState.current);
        }
      }
    });
