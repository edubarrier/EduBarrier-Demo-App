...
const App = () => {
  ...
  const init = async () => {
    ...
    if (hash && hash.includes('access_token')) {
      ...
      try {
        // Get session (hash already cleared above)
        ...
        if (data?.session?.user) {
          ...
          // Get full user data
          const fullUser = await db.getUser(data.session.user.email);
          ...
          if (fullUser) {
            ...
            // Set success message
            setSuccessMessage('\u{2705} Email confirmed! Welcome to EduBarrier!');
            
            // Wait a moment to show success message
            await new Promise(resolve => setTimeout(resolve, 1500));
            
            // Set user and redirect to appropriate view
            setUser(fullUser);
            ...
            setError('Unable to complete account setup. Please contact support.');
            setLoading(false);
          }
        } else {
          setError('Email confirmation failed. Please try logging in.');
        ...
        setError('An error occurred during email confirmation. Please try logging in.');
        setLoading(false);
      }
