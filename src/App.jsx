...
const ChildView = ({ user, onLogout }) => {
  ...
  const loadData = async () => {
    ...
    console.log('Child settings:', childSettings);
    
    // AUTO-LOCK CHECK: If child has 0 time remaining, they should be locked
    // IMPORTANT: Only auto-lock on INITIAL page load, not on subsequent polls
    // This allows parent to manually override (unlock child at 0 time)
    const timeRemaining = t.total - t.used;
    const currentLockStatus = childSettings?.is_locked || childSettings?.isLocked || false;
    
    console.log('Time remaining:', timeRemaining);
    console.log('Current lock status:', currentLockStatus);
    console.log('Has auto-locked before?', hasAutoLockedOnLoad);
    console.log('Should auto-lock?', timeRemaining <= 0 && !currentLockStatus && !hasAutoLockedOnLoad);
    
    // Only auto-lock if:
    // 1. Time remaining is 0
    // 2. Not currently locked
    // 3. Haven't auto-locked on this page load yet (allows parent override)
    if (timeRemaining <= 0 && !currentLockStatus && !hasAutoLockedOnLoad) {
      console.log('⚠️ AUTO-LOCKING CHILD: 0 time remaining on load');
      try {
        ...
        setIsLocked(true);
        setHasAutoLockedOnLoad(true);  // Mark that we've auto-locked
        console.log('✓ Auto-lock successful');
      } catch (error) {
        console.error('❌ Auto-lock failed:', error);
      }
    } else if (timeRemaining <= 0 && !currentLockStatus && hasAutoLockedOnLoad) {
      console.log('ℹ️ Parent manually unlocked at 0 time - respecting override');
    }
    console.log('=== END AUTO-LOCK CHECK ===');
        ...
        <div className="bg-white rounded-3xl shadow-lg p-6">
          ...
          {isLocked && remaining === 0 && (
            <div className="bg-orange-100 border-2 border-orange-300 rounded-xl p-4 mb-4">
              <div className="flex items-center gap-3">
                <div className="text-3xl text-orange-600 font-bold">⏱️</div>
                <div className="flex-1">
