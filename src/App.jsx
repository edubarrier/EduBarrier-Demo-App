...
const ParentView = ({ user, onLogout }) => {
  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    // Only re-render if the child's actual data changes, ignore function references
    return (
      prevProps.child.id === nextProps.child.id &&
      prevProps.child.name === nextProps.child.name &&
      prevProps.child.email === nextProps.child.email &&
      prevProps.editingChild?.id === nextProps.editingChild?.id &&
      JSON.stringify(prevProps.assigns.filter(a => a.childId === prevProps.child.id)) === 
      JSON.stringify(nextProps.assigns.filter(a => a.childId === nextProps.child.id))
    );
  });
