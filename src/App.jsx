...
const AdminView = ({ user, onLogout }) => {
  ...
  const deleteUser = async (userId) => {
    ...
    try {
      ...
      if (userToDelete.family_id) {
        const family = await db.getFamily(userToDelete.family_id);
        if (family) {
          await db.updateFamily(userToDelete.family_id, family);
        }
      }
      ...
      // Reload all users from database to confirm deletion
