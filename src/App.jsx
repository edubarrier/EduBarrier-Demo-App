...
const ParentView = ({ user, onLogout }) => {
  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    const hasOptimisticLockUpdate = useRef(false);
    const hasOptimisticTimeUpdate = useRef(false);
    const hasOptimisticExtraCreditUpdate = useRef(false);
    const hasLoadedRef = useRef(false);
    
    // Store the intended state values in refs so they persist across parent re-renders
    const intendedLockState = useRef(null);
    const intendedTimeEarned = useRef(null);
    const intendedExtraCreditState = useRef(null);
    
    // COMPUTED DISPLAY VALUE: Use intended state if optimistic update is active
    // This runs DURING render, not after, preventing flicker
    const displayLocked = (hasOptimisticLockUpdate.current && intendedLockState.current !== null) 
      ? intendedLockState.current 
      : isLocked;
    ...
    useEffect(() => {
      ...
      const checkStatusAndTime = async () => {
        ...
        // Only update state if user hasn't made optimistic changes
        if (!hasOptimisticLockUpdate.current) {
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    // Calculate and store the intended state in ref (persists across re-renders)
                    const newLockState = !displayLocked;
                    intendedLockState.current = newLockState;
                    ...
                    // Always re-enable button after 1 second, then clear optimistic flag after 3 seconds
                    setTimeout(() => {
                      isProcessingRef.current = false;
                      setIsProcessingUI(false);
                    }, 1000);
                    // Clear the optimistic flag after 3 seconds to allow fresh data
                    setTimeout(() => {
                      hasOptimisticLockUpdate.current = false;
                      intendedLockState.current = null;
                    }, 3000);
                  }
                }}
