...
const ParentView = ({ user, onLogout }) => {
  ...
  const deleteAssignment = async (assignmentId) => {
    ...
    try {
      ...
      const updated = assigns.filter(a => a.id !== assignmentId);
      setAssigns(updated);
      
      setMessage('Assignment deleted successfully!');
      setTimeout(() => setMessage(''), 3000);
      
      console.log('Assignment deleted and state updated');
    } catch (error) {
  ...
  const ChildCard = ({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime }) => {
    ...
    useEffect(() => {
      ...
      const checkStatusAndTime = async () => {
        ...
        setIsLocked(settings?.isLocked || settings?.is_locked || false);
        setActualTimeEarned(settings?.timeEarned || settings?.time_earned || 0);
      };
