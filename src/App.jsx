...
const ParentView = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    ...
    loadFamilyHeartbeats();
    
    // Then poll every 30 seconds (matches child heartbeat interval)
    const interval = setInterval(() => {
      loadFamilyHeartbeats();
    }, 30000);
    
    return () => clearInterval(interval);
  }, [barrierMonitoring]); // Simplified dependencies
  ...
  const ChildHeartbeatCard = ({ child, heartbeat, checkHeartbeatStatus, getTimeSinceHeartbeat, barrierActive }) => {
    ...
    return (
      <div className={`${bgColor} rounded-xl p-5 border-2 ${borderColor}`}>
        ...
        <div className="bg-gray-900 text-green-400 rounded-lg p-4 font-mono text-xs min-h-[180px]">
