...
const ParentView = ({ user, onLogout }) => {
  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    // Debounce timer ref for state reload
    const reloadTimerRef = useRef(null);
    
    // CRITICAL FIX: Single function to reload ALL state at once to prevent cascading updates
    ...
    // CRITICAL FIX: Debounced reload - only reload ONCE after all processing is complete
    const scheduleStateReload = () => {
      // Clear any existing reload timer
      if (reloadTimerRef.current) {
        clearTimeout(reloadTimerRef.current);
      }
      
      // Schedule a single reload after 1.5 seconds
      reloadTimerRef.current = setTimeout(async () => {
        await reloadAllState();
        setIsProcessing(false); // Re-enable all buttons together
      }, 1500);
    };
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    // Schedule a single debounced reload for ALL state
                    scheduleStateReload();
                  } catch (error) {
