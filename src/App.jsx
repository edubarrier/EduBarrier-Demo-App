...
const ParentView = ({ user, onLogout }) => {
  ...
  // Memoize kids array to prevent ChildCard re-renders when parent component updates
  // Only create new array reference when kids actually change (by ID list)
  const memoizedKids = useMemo(() => kids, [kids.map(k => k.id).join(',')]);
  ...
  const updateChildInfo = useCallback(async (childId, updates) => {
    try {
  ...
  }, [user.family_id, user.familyId]); // Depend on user family ID
  ...
  // Wrap toggle functions in useCallback to prevent ChildCard re-renders
  const toggleChildLock = useCallback(async (childId) => {
    try {
      // Get current settings from database
      const currentSettings = await db.getChildSettings(childId);
      
      if (!currentSettings) {
        throw new Error('No settings found for child');
      }
      ...
      // CRITICAL FIX: Include ALL fields to prevent resetting other values
      await db.updateChildSettings(childId, {
        ...
        extra_credit_enabled: currentSettings.extra_credit_enabled || currentSettings.extraCreditEnabled || false // FIXED: Preserve extra credit state
      });
      
      // Optional: Show message to parent
      const message = newLockState ? 'Child screen access locked' : 'Child screen access unlocked';
      setMessage(message);
      setTimeout(() => setMessage(''), 2000);
    } catch (error) {
      console.error('Error toggling lock:', error);
      throw error; // Let ChildCard handle the error
    }
  }, []); // No dependencies - function is stable

  const clearChildTime = useCallback(async (childId) => {
    ...
    // Get current settings to preserve extra_credit_enabled
    const currentSettings = await db.getChildSettings(childId);
  ...
  const toggleExtraCredit = useCallback(async (childId) => {
    try {
      // Get current settings from database
      const currentSettings = await db.getChildSettings(childId);
      ...
      // Update in database - simple and fast
      await db.updateChildSettings(childId, {
      ...
      // Optional: Show message to parent
      const message = newExtraCreditState ? 'Extra Credit enabled' : 'Extra Credit disabled';
      setMessage(message);
      setTimeout(() => setMessage(''), 2000);
    } catch (error) {
    ...
    return (
      <div className={`${bgColor} rounded-xl p-5 border-2 ${borderColor}`}>
        ...
        <div className="bg-gray-900 text-green-400 rounded-lg p-4 font-mono text-xs min-h-[200px]">
          <div className="font-bold mb-3 text-white">📡 HEARTBEAT MONITOR</div>
          <div className="border-t border-gray-700 pt-3 space-y-2">
            <div className="flex justify-between">
              <span>Status:</span>
              <span className={isOnline ? 'text-green-400' : isOffline ? 'text-red-400' : 'text-gray-400'}>
                {isOnline ? '✓ ALIVE' : isOffline ? '✗ NO RESPONSE' : '⏳ WAITING'}
              </span>
            </div>
            <div className="flex justify-between">
  ...
  // Separate component for child card - SIMPLE approach with no polling or optimistic updates
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    // Simple state - loads ONCE and only updates from button clicks
    ...
    // Load initial state ONCE when component mounts or child.id changes
    useEffect(() => {
      let isMounted = true;
      ...
      const loadState = async () => {
        try {
          ...
          if (settings && isMounted) {
            setIsLocked(settings.is_locked || settings.isLocked || false);
            setActualTimeEarned(settings.time_earned || settings.timeEarned || 0);
            setExtraCreditEnabled(settings.extra_credit_enabled || settings.extraCreditEnabled || false);
          }
        } catch (error) {
        ...
        {isEditing ? (
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              {/* Lock/Unlock Button - SIMPLE: Click -> Update DB -> Reload State */}
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    setTimeout(() => setIsProcessingLock(false), 800);
                  }
                }}
              ...
              >
                {isProcessingLock ? 'Processing...' : isLocked ? 'Unlock Screen Access' : 'Lock Screen Access'}
              </button>
              
              {/* Clear Time Button - SIMPLE: Click -> Update DB -> Reload State */}
              <button
                onClick={async () => {
                  if (isProcessingTime) return;
                  ...
                  try {
                    ...
                    setTimeout(() => setIsProcessingTime(false), 800);
                  }
                }}
                disabled={isProcessingTime}
                className={`w-full py-2 rounded-lg text-sm font-bold transition-all ${
                  isProcessingTime 
                    ? 'opacity-50 cursor-not-allowed bg-gray-400 text-gray-600' 
              ...
              >
                {isProcessingTime ? 'Processing...' : 'Clear Screen Time'}
              </button>
              
              {/* Extra Credit Toggle Button - SIMPLE: Click -> Update DB -> Reload State */}
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    console.error('Extra credit toggle failed:', error);
                  } finally {
                    setTimeout(() => setIsProcessingCredit(false), 800);
                  }
                }}
              ...
              >
                {isProcessingCredit ? 'Processing...' : `Extra Credit: ${extraCreditEnabled ? 'ON' : 'OFF'}`}
              </button>
    ...
    // CRITICAL: ONLY re-render if we're showing a completely different child OR editing state changed
    // Return TRUE = skip re-render (props considered equal)
    // Return FALSE = do re-render (props changed)
    
    // Different child? Must re-render
    if (prevProps.child.id !== nextProps.child.id) {
      return false;
    }
    
    // Check if THIS child's editing state changed
    const wasEditing = prevProps.editingChild?.id === prevProps.child.id;
    const isEditing = nextProps.editingChild?.id === nextProps.child.id;
    if (wasEditing !== isEditing) {
      return false; // Editing state changed - must re-render
    }
    
    // Same child, same editing state? ALWAYS skip re-render
    // This prevents flickering from parent updates (heartbeats, assigns, etc.)
    return true;
  });
            ...
            <div className="bg-white rounded-3xl shadow-lg p-6">
              ...
              <div className="flex flex-col items-center mb-8">
                <div className="text-center mb-6">
                  <span className="text-8xl mb-2 block">{barrierStatus?.is_active ? '🛡️' : '🛡️'}</span>
