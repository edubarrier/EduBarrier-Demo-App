...
const ParentView = ({ user, onLogout }) => {
  ...
  const toggleChildLock = async (childId) => {
    ...
    await db.updateChildSettings(childId, {
      ...
      timer_running: newLockState ? false : currentSettings.timerRunning, // Pause timer if locking
...
const ChildView = ({ user, onLogout }) => {
  ...
  const calcResults = async (ans, course) => {
    ...
    console.log('Waited 100ms for database commit');
    
    // Immediately reload assignments to update UI
    const familyId = user.family_id || user.familyId;
    console.log('Reloading assignments for family:', familyId);
    
    const allAssignments = await db.getAssignmentsByFamily(familyId) || [];
    console.log('Total family assignments loaded:', allAssignments.length);
    ...
    console.log('My assignments after filtering:', myAssignments.length);
    console.log('My assignments details:', myAssignments);
    
    const pendingAfterUpdate = myAssignments.filter(a => a.status === 'pending');
    const completedAfterUpdate = myAssignments.filter(a => a.status === 'completed');
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="bg-white rounded-3xl shadow-lg p-6">
          ...
          {remaining > 0 && !isLocked && (
            ...
            >
              {isTimerRunning ? 'Ã¢ÂÂ¸Ã¯Â¸Â Pause Timer' : 'Ã¢â€“Â¶Ã¯Â¸Â Start Timer'}
            </button>
          )}
