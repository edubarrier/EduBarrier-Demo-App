  ...
  const load = async () => {
    try {
      ...
      if (f) {
        ...
        // Get all children in this family from database
        const allUsers = await db.getAllUsers();
        const childUsers = allUsers.filter(u => f.children && f.children.includes(u.id));
        setKids(childUsers);
      } else {
        console.warn('No family data found for:', user.familyId);
      }
      
      // Load assignments from database
      const a = await db.getAssignmentsByFamily(user.familyId) || [];
      setAssigns(a);
  ...
  const deleteUser = async (userId) => {
    ...
    try {
      ...
      try {
        ...
        console.log('Could not delete from Supabase Auth (may not have admin access):', authErr.message);
      }
      
      // Delete from localStorage
      const localUsers = await storage.get('users') || [];
      const filteredLocal = localUsers.filter(u => u.id !== userId && u.email !== userToDelete.email);
      await storage.set('users', filteredLocal);
      
      // If user has a family, remove them from family
      if (userToDelete.familyId) {
        const family = await db.getFamily(userToDelete.familyId);
        if (family) {
          if (userToDelete.role === 'parent') {
            family.parents = family.parents.filter(p => p !== userId);
          } else if (userToDelete.role === 'child') {
            family.children = family.children.filter(c => c !== userId);
          }
          await db.updateFamily(family);
          
          // Also update in localStorage
          await storage.set('fam-' + userToDelete.familyId, family);
        }
      }
      
      // Delete child settings if child
      if (userToDelete.role === 'child') {
        try {
          await db.deleteChildSettings(userId);
          await storage.remove(`child-settings-${userId}`);
        } catch (settingsErr) {
          console.log('Error deleting child settings:', settingsErr);
        }
      }
      
      const updated = users.filter(u => u.id !== userId);
      setUsers(updated);
      setMessage('User completely deleted from all systems!');
      setTimeout(() => setMessage(''), 3000);
    } catch (err) {
