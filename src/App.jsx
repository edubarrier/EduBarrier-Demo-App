...
const ParentView = ({ user, onLogout }) => {
  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    // Use refs to track previous values and prevent flicker
    const prevValuesRef = useRef({ isLocked: false, timeEarned: 0, extraCredit: false });
    ...
    useEffect(() => {
      ...
      const checkStatusAndTime = async () => {
        ...
        // Only update state if values ACTUALLY changed (compare with ref, not prev state)
        if (prevValuesRef.current.isLocked !== newLockStatus) {
          prevValuesRef.current.isLocked = newLockStatus;
          setIsLocked(newLockStatus);
        }
        
        if (prevValuesRef.current.timeEarned !== newTimeEarned) {
          prevValuesRef.current.timeEarned = newTimeEarned;
          setActualTimeEarned(newTimeEarned);
        }
        
        if (prevValuesRef.current.extraCredit !== newExtraCreditStatus) {
          prevValuesRef.current.extraCredit = newExtraCreditStatus;
    ...
    // Only re-render if the specific child's data or relevant props changed
    
    // Check if child object changed
    if (prevProps.child.id !== nextProps.child.id) return false;
    if (prevProps.child.name !== nextProps.child.name) return false;
    if (prevProps.child.email !== nextProps.child.email) return false;
    
    // Check if editing state changed for THIS child
    if (prevProps.editingChild?.id !== nextProps.editingChild?.id) return false;
    
    // Check if assignments for THIS child changed
    const prevChildAssigns = prevProps.assigns.filter(a => a.childId === prevProps.child.id);
    const nextChildAssigns = nextProps.assigns.filter(a => a.childId === nextProps.child.id);
    
    if (prevChildAssigns.length !== nextChildAssigns.length) return false;
    
    // Check if any assignment details changed
    for (let i = 0; i < prevChildAssigns.length; i++) {
      if (prevChildAssigns[i].status !== nextChildAssigns[i].status) return false;
      if (prevChildAssigns[i].mins !== nextChildAssigns[i].mins) return false;
    }
    
    // If all checks passed, props are the same - skip re-render
    return true;
  });

  // PERFORMANCE FIX: Memoize assigns array to prevent ChildCard re-renders
  // Only recalculate if assigns array content actually changes
  const memoizedAssigns = useMemo(() => assigns, [JSON.stringify(assigns)]);
