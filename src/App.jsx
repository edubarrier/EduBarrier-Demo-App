...
import { Book, Users, Trophy, XCircle, Plus, LogOut, Eye, EyeOff, Trash2, ChevronLeft, ChevronRight } from 'lucide-react';
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md">
          ...
          <div className="mt-6 pt-6 border-t">
            ...
            <div className="text-sm text-gray-600 space-y-1">
              ...
              <p><span className="font-medium">Child:</span> demo@child.com / demo</p>
...
const ParentView = ({ user, onLogout }) => {
  ...
  // NEW: Calendar feature state variables
  const [selectedCalendarDate, setSelectedCalendarDate] = useState(null);
  const [currentMonth, setCurrentMonth] = useState(new Date().getMonth());
  const [currentYear, setCurrentYear] = useState(new Date().getFullYear());
  const [assignDate, setAssignDate] = useState(new Date().toISOString().split('T')[0]); // For assignment form
  ...
  const doAssign = async () => {
    ...
    const a = {
      ...
      status: 'pending',
      score: null,
      assigned_date: assignDate  // NEW: Include the date when assigning
    };
  ...
  const toggleChildLock = async (childId) => {
    ...
    const newLockState = !currentSettings.isLocked;
    
    // Update in database for real-time sync across accounts
    await db.updateChildSettings({
      childId,
      isLocked: newLockState,
      timerRunning: newLockState ? false : currentSettings.timerRunning, // Pause timer if locking
      timeEarned: currentSettings.timeEarned,
      timeUsed: currentSettings.timeUsed
    });
    
    setMessage(newLockState ? 'Child screen access locked' : 'Child screen access unlocked');
    setTimeout(() => setMessage(''), 3000);
    
    // Update the kids state directly instead of full reload to prevent button glitch
    setKids(prevKids => prevKids.map(k => k.id === childId ? k : k));
  };

  // NEW: Delete assignment function for calendar feature
  const deleteAssignment = async (assignmentId) => {
    if (!confirm('Are you sure you want to delete this assignment? The child will no longer see this course.')) {
      return;
    }
    ...
    try {
      ...
      // Update local state
      setAssigns(prevAssigns => prevAssigns.filter(a => a.id !== assignmentId));
            ...
            <div className="bg-white rounded-3xl shadow-lg p-6">
              ...
              <div className="mb-6">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setSelectedCalendarDate(null); // Reset selected date when changing child
                  }}
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-teal-500 focus:outline-none"
                >
              ...
              {calendarChild ? (
                (() => {
                  ...
                  // Helper to check if a date has assignments
                  const getAssignmentsForDate = (day) => {
                    if (!day) return [];
                    const dateStr = `${currentYear}-${String(currentMonth + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
                    return childAssigns.filter(a => {
                      const assignDate = a.assigned_date || a.assignedDate;
                      return assignDate === dateStr;
                    });
                  };
                  
                  // Get assignments for selected date
                  const selectedDateAssignments = selectedCalendarDate 
                    ? getAssignmentsForDate(selectedCalendarDate)
                    : [];
                  ...
                  return (
                    <div>
                      <div className="bg-gradient-to-br from-teal-50 to-emerald-50 rounded-2xl p-6 mb-6">
                        ...
                        <p className="text-sm text-gray-600">Click on any date to view or manage assignments</p>
                      </div>
                              ...
                              <button
                                ...
                                className={`aspect-square flex flex-col items-center justify-center rounded-lg text-sm font-medium transition-all relative ${
                                  isSelected 
                                    ...
                                    : isToday 
                                    ? 'bg-teal-100 text-teal-800 font-bold border-2 border-teal-600' 
                                    : hasAssignments
                      ...
                      {selectedCalendarDate && (
                        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border-2 border-blue-200">
                          <h4 className="text-xl font-bold text-gray-800 mb-2">
                            Assignments for {monthNames[currentMonth]} {selectedCalendarDate}, {currentYear}
                          </h4>
                          <p className="text-sm text-gray-600 mb-4">
                            Click the trash icon to delete an assignment
                          </p>

                          {selectedDateAssignments.length === 0 ? (
                            <div className="text-center py-8 bg-white rounded-xl">
                              <Book className="mx-auto text-gray-300 mb-2" size={48} />
                              <p className="text-gray-600">No courses assigned for this date</p>
                              <button
                                onClick={() => {
                                  setSelKid(calendarChild);
                                  setAssignDate(`${currentYear}-${String(currentMonth + 1).padStart(2, '0')}-${String(selectedCalendarDate).padStart(2, '0')}`);
                                  setTab('schedule');
                                }}
                                className="mt-4 bg-teal-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-teal-700"
                              >
                                Assign Course for This Date
                              </button>
                            </div>
                          ) : (
                            <div className="space-y-3">
                                  ...
                                  >
                                    <div className="flex justify-between items-start">
                                      <div className="flex-1">
                                        <h5 className="font-bold text-gray-800 text-lg">{course?.title}</h5>
                                        <p className="text-sm text-gray-600">{course?.category}</p>
                                        <p className="text-xs text-gray-500 mt-1">
                                          Reward: {a.mins} minutes | Passing Score: {a.passScore || a.pass_score}%
                                        </p>
                                      </div>
                                      <div className="flex items-center gap-3">
                                        <span
                                        ...
                                        >
                                          <Trash2 size={20} className="text-red-500 group-hover:text-red-700" />
                                        </button>
                                      </div>
                                    </div>
                                  </div>
                      ...
                      {/* All Assignments Overview (when no date selected) */}
                      {!selectedCalendarDate && (
                        <div>
                          <h4 className="text-lg font-bold text-gray-800 mb-4">All Assigned Tasks</h4>
                          {childAssigns.length === 0 ? (
                            ...
                            <div className="space-y-3">
                              {childAssigns.slice().sort((a, b) => {
                                const dateA = a.assigned_date || a.assignedDate || '9999-12-31';
                                const dateB = b.assigned_date || b.assignedDate || '9999-12-31';
                                  ...
                                  >
                                    <div className="flex justify-between items-start">
                                      <div className="flex-1">
                                        ...
                                        <p className="text-sm text-gray-600">{course?.category}</p>
                                        <p className="text-xs text-gray-500 mt-1">
                                          Due: {assignDate ? new Date(assignDate + 'T00:00:00').toLocaleDateString() : 'Not set'}
                                        </p>
                                        <p className="text-xs text-gray-500">
                                          Reward: {a.mins} minutes | Passing: {a.passScore || a.pass_score}%
                                        </p>
                                      </div>
                                      <div className="flex items-center gap-3">
                                        <span
                      ...
                      >
                        {selectedCalendarDate ? `Assign Course for ${monthNames[currentMonth]} ${selectedCalendarDate}` : 'Assign New Course'}
                      </button>
                    </div>
                  );
                })()
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-3xl shadow-lg p-6">
              ...
              <div className="space-y-4">
                ...
                {/* NEW: Date Picker for Assignment */}
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-teal-500 focus:outline-none" 
                  />
                  <p className="text-xs text-gray-500 mt-1">Select when this course should be completed</p>
                </div>
...
const ChildView = ({ user, onLogout }) => {
  ...
  // Check lock status periodically from database for real-time sync
  React.useEffect(() => {
    ...
    checkLock();
    const interval = setInterval(checkLock, 3000); // Check every 3 seconds for real-time sync
  ...
  const calcResults = async (ans, course) => {
    ...
    const assignmentData = {
      ...
      score: score,
      child_id: active.child_id || active.childId,
      family_id: active.family_id || active.familyId,
      course_id: active.course_id || active.courseId,
      pass_score: passScore
    };
    
    await db.updateAssignment(assignmentData);
    ...
    if (passed) {
      ...
      await db.updateChildSettings({
        childId: user.id,
        isLocked: settings?.isLocked || false,
        timerRunning: settings?.timerRunning || settings?.timer_running || false,
