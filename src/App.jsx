...
// MODULE-LEVEL CACHE: Stores optimistic lock states that persist across component re-renders/remounts
// Key = childId, Value = { locked: boolean, timestamp: number }
...
const setOptimisticLock = (childId, locked) => {
  ...
  console.log('Cache set:', childId, locked);
};
...
const ParentView = ({ user, onLogout }) => {
    ...
    return (
      <div className={`${bgColor} rounded-xl p-5 border-2 ${borderColor}`}>
        ...
        <div className="bg-gray-900 text-green-400 rounded-lg p-4 font-mono text-xs min-h-[200px]">
          <div className="font-bold mb-3 text-white">ðŸ“¡ HEARTBEAT MONITOR</div>
  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    const hasOptimisticExtraCreditUpdate = useRef(false);
    const hasLoadedRef = useRef(false);
    
    // COMPUTED DISPLAY VALUE: Check module-level cache first (survives component remount)
    ...
    console.log(`ChildCard render: child=${child.id}, cachedLock=${cachedLock}, isLocked=${isLocked}, displayLocked=${displayLocked}`);
    ...
    useEffect(() => {
      ...
      const checkStatusAndTime = async () => {
        ...
        if (!hasOptimisticExtraCreditUpdate.current) {
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                className={`w-full py-2 rounded-lg text-sm font-bold transition-all ${isProcessingUI ? 'opacity-50 cursor-not-allowed' : ''} ${
                  displayLocked 
              ...
              <button
                onClick={async () => {
                  ...
                  setIsProcessingUI(true);
                  try {
                    // Mark that user has made an optimistic update
                    hasOptimisticExtraCreditUpdate.current = true;
                    // Optimistic update: flip the UI immediately
                    setExtraCreditEnabled(prev => !prev);
                    // Then update database in background
              ...
              >
                {extraCreditEnabled ? '\u{2B50} Extra Credit: ON' : '\u{2B50} Extra Credit: OFF'}
              </button>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="Quick">Quick (ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°Ãƒâ€šÃ‚Â¤20 min)</option>
            ...
            <div className="bg-white rounded-3xl shadow-lg p-6">
              ...
              <div className="flex flex-col items-center mb-8">
                <div className="text-center mb-6">
                  <span className="text-8xl mb-2 block">{barrierStatus?.is_active ? 'ðŸ›¡ï¸' : 'ðŸ””'}</span>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <ul className="text-sm text-blue-800 space-y-1">
                      <li>â€¢ Click "Activate Barrier" to start monitoring</li>
                      <li>â€¢ System checks child's app every 30 seconds</li>
                      <li>â€¢ You'll be notified instantly if app is closed</li>
                      <li>â€¢ Child just needs to keep app open - no spam notifications</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="border-b pb-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500">ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â© 2025 EduBarrier Inc. All rights reserved.</p>
                    <button 
                      onClick={() => {
...
const AdminView = ({ user, onLogout }) => {
  ...
  const checkSystemStatus = async () => {
    console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒâ€šÃ‚Â Starting system status check...');
                ...
                <div className={`flex justify-between items-center p-3 rounded-lg ${
                  ...
                  <span className={`font-semibold ${
                    ...
                    {systemStatus.storage.status === 'online' ? '\u{2713} Online' : 
                     systemStatus.storage.status === 'warning' ? 'ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Warning' : 
                     systemStatus.storage.status === 'offline' ? 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂºÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Offline' : '\u{23F3} Checking'}
                  </span>
                </div>
                ...
                <div className={`flex justify-between items-center p-3 rounded-lg ${
                  ...
                  <span className={`font-semibold ${
                    ...
                    {systemStatus.courses.status === 'online' ? '\u{2713} Online' : 
                     systemStatus.courses.status === 'warning' ? 'ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Warning' : 
                     systemStatus.courses.status === 'offline' ? 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂºÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Offline' : '\u{23F3} Checking'}
                  </span>
                </div>
                ...
                <div className={`flex justify-between items-center p-3 rounded-lg ${
                  ...
                  <span className={`font-semibold ${
                    ...
                    {systemStatus.users.status === 'online' ? '\u{2713} Online' : 
                     systemStatus.users.status === 'warning' ? 'ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Warning' : 
                     systemStatus.users.status === 'offline' ? 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂºÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Offline' : '\u{23F3} Checking'}
                  </span>
                </div>
              </div>
            </div>
