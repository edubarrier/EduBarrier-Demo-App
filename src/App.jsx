...
const ParentView = ({ user, onLogout }) => {
  ...
  const lastCheckTimeRef = useRef(null); // Use ref instead of state to prevent re-renders
  ...
  useEffect(() => {
    ...
    const loadHeartbeats = async () => {
      try {
        ...
        // Update last check time using ref (doesn't trigger re-render)
  ...
  const getTimeSinceLastCheck = () => {
    if (!lastCheckTimeRef.current) return 'Not checked yet';
    
    const now = new Date();
    const secondsAgo = Math.floor((now - lastCheckTimeRef.current) / 1000);
  ...
  const toggleChildLock = async (childId) => {
    try {
      ...
      // CRITICAL FIX: Prevent concurrent executions
      if (window.isTogglingLock) {
        console.log('Already toggling lock - ignoring duplicate call');
        return;
      }
      window.isTogglingLock = true;
      ...
      // CRITICAL: Release lock flag after 1 second to allow next toggle
      setTimeout(() => {
        window.isTogglingLock = false;
      }, 1000);
    } catch (error) {
      ...
      // CRITICAL: Release lock flag on error
      window.isTogglingLock = false;
    }
  };
  ...
  const toggleExtraCredit = async (childId) => {
    try {
      // CRITICAL FIX: Prevent concurrent executions
      if (window.isTogglingExtraCredit) {
        console.log('Already toggling extra credit - ignoring duplicate call');
        return;
      }
      window.isTogglingExtraCredit = true;
      ...
      if (!currentSettings) {
        setMessage('Error: Could not load child settings');
        setTimeout(() => setMessage(''), 3000);
        window.isTogglingExtraCredit = false;
        return;
      }
      ...
      // CRITICAL: Release lock flag after 1 second
      setTimeout(() => {
        window.isTogglingExtraCredit = false;
      }, 1000);
    } catch (error) {
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                className={`w-full py-2 rounded-lg text-sm font-bold transition-all ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''} ${
                  isLocked 
              ...
              <button
                ...
                className={`w-full py-2 rounded-lg text-sm font-bold ${isProcessing ? "opacity-50 cursor-not-allowed" : ""} bg-orange-500 text-white hover:bg-orange-600 transition-all`}
              >
                Clear Screen Time
              </button>
