  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    useEffect(() => {
      ...
      const checkStatusAndTime = async () => {
        ...
        // Always update state from database - no optimistic updates to conflict
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    // NO optimistic update - wait for database to prevent flickering
                    await toggleChildLock(child.id);
                    // Reload state from database after toggle
                    const settings = await db.getChildSettings(child.id);
                    if (settings) {
                      setIsLocked(settings.is_locked || settings.isLocked || false);
                    }
                  } finally {
                    // Re-enable button after 1.5 seconds to prevent rapid clicking
