...
const ParentView = ({ user, onLogout }) => {
  ...
  const toggleChildLock = async (childId) => {
    try {
      ...
      console.log('Current extra_credit_enabled:', currentSettings.extra_credit_enabled);
      console.log('Current extraCreditEnabled:', currentSettings.extraCreditEnabled);
      ...
      const updateData = {
        ...
        extra_credit_enabled: currentSettings.extra_credit_enabled || currentSettings.extraCreditEnabled || false
      };
      
      console.log('Updating with:', updateData);
      
      // Update in database
      const result = await db.updateChildSettings(childId, updateData);
        ...
        {isEditing ? (
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    // Reload ALL state from database
                    const settings = await db.getChildSettings(child.id);
                    if (settings) {
                      setIsLocked(settings.is_locked || settings.isLocked || false);
                      setActualTimeEarned(settings.time_earned || settings.timeEarned || 0);
              ...
              >
                {isProcessingCredit ? 'Processing...' : `${extraCreditEnabled ? '\u{2B50} Extra Credit: ON' : '\u{2B50} Extra Credit: OFF'}`}
              </button>
