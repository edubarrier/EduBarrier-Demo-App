...
const App = () => {
  ...
  const handleLogin = async () => {
    ...
    // Special cleanup for problematic account
    if (email === 'coynevideo@gmail.com') {
      const localUsers = await storage.get('users') || [];
      const filtered = localUsers.filter(u => u.email !== 'coynevideo@gmail.com');
      await storage.set('users', filtered);
      setError('Account removed from system. Please create a new account if needed.');
      return;
    }
    ...
    // Demo parent login - ensure in database
    if (email === 'demo@parent.com' && password === 'demo') {
      let demoParent = await db.getUser(email);
      if (!demoParent) {
        demoParent = { id: 'demo', email, role: 'parent', name: 'Demo Parent', familyId: 'fam1' };
        await db.createUser(demoParent);
      }
      
      // Ensure demo family exists in database
      let demoFamily = await db.getFamily('fam1');
      if (!demoFamily) {
        demoFamily = {
          id: 'fam1',
          name: 'Demo Family',
      ...
      } else if (!demoFamily.children.includes('child1')) {
        demoFamily.children.push('child1');
        await db.updateFamily(demoFamily);
      }
    ...
    // Demo child login - ensure in database
    if (email === 'demo@child.com' && password === 'demo') {
      let demoChild = await db.getUser(email);
      if (!demoChild) {
        demoChild = { id: 'child1', email, role: 'child', name: 'Demo Child', familyId: 'fam1' };
        await db.createUser(demoChild);
      }
      
      // Ensure demo family exists
      let demoFamily = await db.getFamily('fam1');
      if (!demoFamily) {
        demoFamily = {
          id: 'fam1',
      ...
      // Initialize child settings in database
      const childSettings = await db.getChildSettings('child1');
      if (!childSettings) {
    ...
    try {
      // First, try to find user in database with direct password check
      ...
      if (data.user) {
        ...
        const dbUserFromAuth = await ensureUserInDatabase(data.user);
      ...
      // Last resort: check localStorage and migrate to database
      if (authError) {
        console.log('Checking localStorage for legacy accounts...');
        
        const localUsers = await storage.get('users') || [];
        const found = localUsers.find(u => u.email === email && u.password === password);
        
        if (found) {
          console.log('Found legacy user, migrating to database:', found.email);
          
          try {
            // Create user in database
            await db.createUser(found);
            
            // Migrate family if exists
            if (found.familyId) {
              const localFamily = await storage.get('fam-' + found.familyId);
              if (localFamily) {
                const dbFamily = await db.getFamily(found.familyId);
                if (!dbFamily) {
                  await db.createFamily(localFamily);
                  console.log('Migrated family to database:', localFamily.id);
                }
              }
            }
            
            // Migrate child settings if child account
            if (found.role === 'child') {
              const localSettings = await storage.get(`child-settings-${found.id}`);
              if (localSettings) {
                await db.updateChildSettings(localSettings);
                console.log('Migrated child settings to database:', found.id);
              }
            }
            
            // Now log in with the migrated account
            await storage.set('session', found);
            setUser(found);
            setView(found.role === 'parent' ? 'parent' : 'child');
            setSuccessMessage('Account migrated to database successfully!');
            return;
          } catch (migrateErr) {
            console.error('Error migrating user:', migrateErr);
            setError('Error migrating account. Please contact support.');
            return;
          }
        }
      }
