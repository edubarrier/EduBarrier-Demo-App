...
const App = () => {
  ...
  const init = async () => {
    try {
      ...
      if (!courses || courses.length === 0) {
        // Initialize default courses in Supabase
      ...
      // Restore session if it exists (fixes refresh issue)
  ...
  const handleLogin = async () => {
    ...
    // Demo account - check Supabase, create if needed
    if (email === 'demo@parent.com' && password === 'demo') {
      try {
        let demoUser = await db.getUser(email, password);
        ...
        // Update session with family_id
        const sessionUser = { ...demoUser, familyId: demoUser.family_id };
        await storage.set('session', sessionUser);
        setUser(sessionUser);
        setView('parent');
        return;
      } catch (error) {
    ...
    // Regular user login - check Supabase
    try {
      const found = await db.getUser(email, password);
      
      if (found) {
        const sessionUser = { ...found, familyId: found.family_id };
        await storage.set('session', sessionUser);
        setUser(sessionUser);
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      const existingUser = await db.getUser(email, password);
      if (existingUser) {
      ...
      if (type === 'parent') {
        if (householdAction === 'create') {
          ...
          const sessionUser = { ...newUser, familyId: fid };
          await storage.set('session', sessionUser);
          setUser(sessionUser);
          setView('parent');
        } else {
          ...
          const foundFamily = await db.getFamilyByCode(code);
          ...
          newUser.family_id = foundFamily.id;
          
          // Create user in database
          await db.createUser(newUser);
          
          const sessionUser = { ...newUser, familyId: foundFamily.id };
          await storage.set('session', sessionUser);
          setUser(sessionUser);
          setView('parent');
        }
        ...
        await db.updateChildSettings(newUser.id, {
          is_locked: true,
          timer_running: false,
          time_earned: 0,
          time_used: 0
  ...
  const handleLogout = async () => {
    await storage.remove('session'); // Changed from set to remove
    setUser(null);
    setView('login');
    setEmail('');
    setPassword('');
