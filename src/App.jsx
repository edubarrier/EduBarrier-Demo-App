...
const ParentView = ({ user, onLogout }) => {
  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    // Store child ID to detect if we're rendering a different child
    const childIdRef = useRef(child.id);
    
    useEffect(() => {
      // If child ID changed, reset the loaded flag
      if (childIdRef.current !== child.id) {
        childIdRef.current = child.id;
        hasLoadedRef.current = false;
      }
      
      // Only load ONCE on first mount - never reload from props
      ...
      loadInitialState();
    }, [child.id]); // ONLY re-run if child.id changes (different child)
