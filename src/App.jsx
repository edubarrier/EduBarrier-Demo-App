...
const ParentView = ({ user, onLogout }) => {
  ...
  const toggleChildLock = async (childId) => {
    try {
      ...
      // ONLY update lock and timer fields - leave everything else alone!
      const updateData = {
  ...
  const clearChildTime = async (childId) => {
    ...
    // ONLY update time and lock fields - leave extra_credit_enabled alone!
  ...
  const toggleExtraCredit = async (childId) => {
    try {
      ...
      // ONLY update extra_credit_enabled - leave everything else alone!
      await db.updateChildSettings(childId, {
