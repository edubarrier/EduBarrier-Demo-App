...
const ParentView = ({ user, onLogout }) => {
  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    // CRITICAL FIX: Separate processing states for each button to prevent cross-contamination
    ...
    useEffect(() => {
      // Only load ONCE on first mount - never poll or reload
        ...
        {isEditing ? (
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={async () => {
                  if (isProcessingLock) return; // Prevent double-clicks on THIS button
                  setIsProcessingLock(true);
                  
                  // OPTIMISTIC UPDATE: Flip UI immediately for instant feedback
                  ...
                  try {
                    ...
                    setIsLocked(!newLockState); // Flip back
                  } finally {
              ...
              <button
                onClick={async () => {
                  if (isProcessingTime) return; // Prevent double-clicks on THIS button
                  ...
                  try {
                    ...
                    // On error, we can't revert since we don't know previous values
                    // Just log the error - page refresh will show correct state
              ...
              <button
                onClick={async () => {
                  if (isProcessingCredit) return; // Prevent double-clicks on THIS button
                  setIsProcessingCredit(true);
                  
                  // OPTIMISTIC UPDATE: Flip UI immediately for instant feedback
                  const newCreditState = !extraCreditEnabled;
                  setExtraCreditEnabled(newCreditState);
                  ...
                  try {
                    ...
                    setExtraCreditEnabled(!newCreditState); // Flip back
                  } finally {
                    // Re-enable button after 800ms
