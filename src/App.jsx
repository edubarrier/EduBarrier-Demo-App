...
const ParentView = ({ user, onLogout }) => {
  ...
  const toggleChildLock = async (childId) => {
    ...
    // Check BOTH field names for current state
    const currentLockState = currentSettings.isLocked || currentSettings.is_locked || false;
    const newLockState = !currentLockState;
    
    console.log('Toggle lock - Child ID:', childId, 'Current:', currentLockState, 'New:', newLockState);
    
    // Update in database for real-time sync across accounts
    await db.updateChildSettings(childId, {
      is_locked: newLockState,
      timer_running: newLockState ? false : (currentSettings.timerRunning || currentSettings.timer_running || false),
      time_earned: currentSettings.timeEarned || currentSettings.time_earned || 0,
      time_used: currentSettings.timeUsed || currentSettings.time_used || 0
    });
    
    console.log('Lock updated in database');
    
    setMessage(newLockState ? 'Child screen access locked' : 'Child screen access unlocked');
    setTimeout(() => setMessage(''), 3000);
    
    // Don't reload kids - let the polling interval handle UI updates
  ...
  const ChildCard = ({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime }) => {
    ...
    useEffect(() => {
      ...
      const checkLockStatus = async () => {
        ...
        const locked = settings?.isLocked || settings?.is_locked || false;
        console.log('ChildCard lock check - Child:', child.name, 'Settings:', settings, 'Locked:', locked);
        setIsLocked(locked);
      };
            ...
            <div className="bg-white rounded-3xl shadow-lg p-6">
              ...
              <div className="mb-6">
                ...
                <select
                  ...
                  className="w-full px-4 py-3 border-2 border-teal-200 rounded-xl focus:border-teal-500 focus:outline-none text-lg"
                >
              ...
              {!calendarChild ? (
                <div className="text-center py-20 bg-gray-50 rounded-xl">
                  <div className="text-gray-300 mb-4">
                    <svg className="mx-auto" width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                ...
                <>
                  <h3 className="text-xl font-bold text-gray-800 mb-4">
                    {kids.find(k => k.id === calendarChild)?.name}'s Assignment Calendar
                  </h3>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <button
                      ...
                      className="px-4 py-2 bg-teal-100 text-teal-700 rounded-lg hover:bg-teal-200 font-semibold"
                    >
                      ← Previous
                    </button>
                  ...
                  <div className="grid grid-cols-7 gap-2 mb-6">
                    ...
                    {(() => {
                      ...
                      // Filter assignments for selected child
                      ...
                      for (let day = 1; day <= daysInMonth; day++) {
                        ...
                        const dayAssignments = childAssigns.filter(a => {
                          if (!a.created_at && !a.createdAt) return false;
                          const assignDate = new Date(a.created_at || a.createdAt);
                          return assignDate.toISOString().split('T')[0] === dateStr;
                        });
                            ...
                            <div className="flex flex-col items-center justify-center h-full">
                              ...
                              {dayAssignments.length > 0 && (
                                <span className="text-xs mt-1">
                                  {isSelected ? '●' : dayAssignments.length}
                                </span>
                              )}
                            </div>
                          </button>
                  ...
                  {selectedDateStr && (
                    <div className="border-t-2 border-gray-200 pt-6">
                      <h4 className="text-lg font-bold text-gray-800 mb-4">
                        Assignments for {new Date(selectedDateStr + 'T12:00:00').toLocaleDateString('en-US', { 
                          weekday: 'long', 
                          year: 'numeric', 
                          month: 'long', 
                          day: 'numeric' 
                      ...
                      {(() => {
                        ...
                        const dayAssignments = childAssigns.filter(a => {
                          if (!a.created_at && !a.createdAt) return false;
                          const assignDate = new Date(a.created_at || a.createdAt);
                          return assignDate.toISOString().split('T')[0] === selectedDateStr;
                        });
                        
                        if (dayAssignments.length === 0) {
                          return (
                            <div className="text-center py-12 bg-gray-50 rounded-xl">
                              <p className="text-gray-600">No assignments on this date</p>
                            </div>
                          );
                        }
                        ...
                        return (
                          <div className="space-y-3">
                            {dayAssignments.map(a => {
                              ...
                              const course = courses.find(c => c.id === a.courseId);
                              return (
                                <div key={a.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-teal-50 to-emerald-50 rounded-xl border-2 border-teal-100">
                                  <div className="flex-1">
                                    ...
                                    <div className="flex gap-2 text-sm flex-wrap">
                                      <span className={`px-3 py-1 rounded-full font-medium ${
                                        ...
                                        {a.status === 'completed' ? `Completed - ${a.score}%` : 'In Progress'}
                                      </span>
                                      ...
                                      {a.status === 'completed' && a.completed_at && (
                                        <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-medium">
                                          {new Date(a.completed_at).toLocaleTimeString('en-US', { 
                                            hour: 'numeric', 
                                            minute: '2-digit' 
                                          })}
                                        </span>
                                  ...
                                  <button
                                    ...
                                    className="ml-4 text-red-500 hover:text-white bg-red-50 hover:bg-red-500 p-3 rounded-xl transition-all flex-shrink-0"
...
const ChildView = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    ...
    // Reload assignments every 3 seconds to see status updates
    const reloadInterval = setInterval(loadData, 3000);
  ...
  React.useEffect(() => {
    const checkLock = async () => {
      ...
      // Check BOTH field names for compatibility
      const locked = settings?.isLocked || settings?.is_locked || false;
      console.log('Lock check - Settings:', settings, 'Locked:', locked);
      setIsLocked(locked);
  ...
  const calcResults = async (ans, course) => {
    ...
    console.log('Assignment ID:', active.id);
    ...
    // Call with (id, updates) format to match supabase.js
    await db.updateAssignment(active.id, assignmentData);
    
    console.log('Assignment updated in database');
    
    // Immediately reload assignments to update UI
    const familyId = user.family_id || user.familyId;
    const allAssignments = await db.getAssignmentsByFamily(familyId) || [];
    console.log('Reloaded assignments from database:', allAssignments);
    ...
    console.log('My assignments after filter:', myAssignments);
    console.log('Pending:', myAssignments.filter(a => a.status === 'pending'));
    console.log('Completed:', myAssignments.filter(a => a.status === 'completed'));
  ...
  const closeAll = () => {
    ...
    setViewing(false);
    setResults(null);
    // Force reload of assignments to show updated status
    loadData();
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50">
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="bg-white rounded-3xl shadow-lg p-6">
          ...
          {remaining > 0 && (
            <button
              onClick={isLocked ? null : toggleTimer}
              disabled={isLocked}
            ...
            >
              {isLocked 
                ? '🔒 Screen Access Locked by Parent' 
                : isTimerRunning 
                  ? '⏸️ Pause Timer' 
                  : '▶️ Start Timer'
              }
