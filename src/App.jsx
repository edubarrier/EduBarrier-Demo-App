...
const ParentView = ({ user, onLogout }) => {
  ...
  const toggleChildLock = async (childId) => {
    try {
      ...
      console.error('âŒ Error toggling lock:', error);
  ...
  const clearChildTime = async (childId) => {
    ...
    // Reset time to zero AND lock the device (since they have no time)
    await db.updateChildSettings(childId, {
      is_locked: true,  // LOCK when clearing time - they have no time left
...
const ChildView = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    ...
    const checkLockStatus = async () => {
      ...
      setIsLocked(prev => {
        if (prev !== lockStatus) {
          console.log('Child lock status changed:', prev, 'â†’', lockStatus);
          return lockStatus;
        }
  ...
  const loadData = async () => {
    ...
    // Auto-lock if time is 0 or negative AND not already locked
    const timeRemaining = t.total - t.used;
    const currentLockState = childSettings?.is_locked || childSettings?.isLocked || false;
    
    if (timeRemaining <= 0 && !currentLockState) {
      console.log('AUTO-LOCK: Time is 0, locking child device');
      ...
      setIsLocked(true);
      console.log('Child device locked due to 0 time');
    } else if (timeRemaining <= 0) {
      console.log('Time is 0 but already locked - no action needed');
    } else {
      console.log('Time remaining > 0, no auto-lock needed');
    }
    
    console.log('NOTE: Parent can still manually unlock at 0 time if desired');
  ...
  const calcResults = async (ans, course) => {
    ...
    if (updated) {
      ...
      console.error('âŒ Database update failed - but continuing with state update');
    }
        ...
        <div className="bg-white rounded-3xl shadow-lg p-6">
          ...
          {isLocked && remaining === 0 && (
            <div className="bg-orange-100 border-2 border-orange-300 rounded-xl p-4 mb-4">
              <div className="flex items-center gap-3">
                <div className="text-3xl text-orange-600 font-bold">â±ï¸</div>
                <div className="flex-1">
...
const AdminView = ({ user, onLogout }) => {
  ...
  const deleteUser = async (userId) => {
    ...
    try {
      ...
      if (deleteError) {
        console.error('Ã¢ÂÅ’ Database delete failed:', deleteError);
      ...
      console.error('Ã¢ÂÅ’ Error deleting user:', err);
