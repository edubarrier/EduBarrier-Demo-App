...
const App = () => {
  ...
  const handleLogin = async () => {
    ...
    // Admin login (legacy - keep working)
    ...
    // Demo login (legacy - keep working)
    if (email === 'demo@parent.com' && password === 'demo') {
      ...
      } else if (!existingFamily.children.includes('child1')) {
        // Ensure demo child is in the family
        existingFamily.children.push('child1');
        await storage.set('fam-fam1', existingFamily);
      }
      setUser(u);
      setView('parent');
      return;
    ...
    if (email === 'demo@child.com' && password === 'demo') {
      const u = { id: 'child1', email, role: 'child', name: 'Demo Child', familyId: 'fam1' };
      await storage.set('session', u);
      
      // Ensure family exists with both parent and child
      let existingFamily = await storage.get('fam-fam1');
      if (!existingFamily) {
        existingFamily = {
          id: 'fam1',
          name: 'Demo Family',
        ...
        // Ensure demo child is in the family
        if (!existingFamily.children.includes('child1')) {
          existingFamily.children.push('child1');
          await storage.set('fam-fam1', existingFamily);
        }
      }
      
      // Initialize child settings if they don't exist
      const childSettings = await storage.get(`child-settings-child1`);
      if (!childSettings) {
        await storage.set(`child-settings-child1`, {
          childId: 'child1',
          isLocked: false,
          timerRunning: false,
          timeEarned: 0,
    ...
    try {
      ...
      if (data.user) {
        ...
        const dbUser = await ensureUserInDatabase(data.user);
      ...
      // If Supabase Auth failed, try legacy authentication
      if (authError) {
        console.log('Supabase auth failed, trying legacy auth:', authError.message);
        
        const users = await storage.get('users') || [];
        const found = users.find(u => u.email === email && u.password === password);
        
        if (found) {
          // Migrate legacy user to database if not already there
          const dbUser = await db.getUser(found.email);
          if (!dbUser) {
            console.log('Migrating legacy user to database:', found.email);
            try {
              await db.createUser(found);
              // If user has a family, ensure it's in database too
              if (found.familyId) {
                const family = await storage.get('fam-' + found.familyId);
                if (family) {
              ...
              // If child, ensure settings are in database
              if (found.role === 'child') {
                const settings = await storage.get(`child-settings-${found.id}`);
                if (settings) {
                  await db.updateChildSettings(settings);
                }
              }
            } catch (migrateErr) {
              console.error('Error migrating user to database:', migrateErr);
            }
          }
