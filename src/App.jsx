    ...
    if (email === 'demo@parent.com' && password === 'demo') {
      try {
        ...
        if (!demoParent) {
          ...
          // Create demo parent user linked to family
          demoParent = { 
            id: 'demo_parent', 
  ...
  const load = async () => {
    try {
      ...
      if (f) {
        ...
        // Get all children in this family from database (by matching familyId in users table)
        const allUsers = await db.getAllUsers();
        const childUsers = allUsers.filter(u => u.familyId === user.familyId && u.role === 'child');
