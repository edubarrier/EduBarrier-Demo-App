...
const ParentView = ({ user, onLogout }) => {
  ...
  const checkHeartbeatStatus = (lastHeartbeat) => {
    ...
    // Consider offline if no heartbeat for 45 seconds (1.5 missed cycles - more responsive)
    if (secondsAgo > 45) return 'offline';
    return 'online';
  };
  ...
  const ChildCard = React.memo(({ child, assigns, editingChild, setEditingChild, updateChildInfo, deleteChild, toggleChildLock, clearChildTime, toggleExtraCredit }) => {
    ...
    useEffect(() => {
      // Check lock status, time earned, AND extra credit status from database ONCE on mount
