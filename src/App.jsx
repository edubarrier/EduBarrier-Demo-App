...
const ParentView = ({ user, onLogout }) => {
  ...
  const deleteAssignment = async (assignmentId) => {
    ...
    try {
      ...
      console.log('Ã¢Å“â€¦ Database deletion returned true');
  ...
  const toggleChildLock = async (childId) => {
    try {
      ...
      console.error('Ã¢ÂÅ’ Error toggling lock:', error);
            ...
            <div className="space-y-2">
              ...
              {isLocked && actualTimeEarned === 0 && (
                <div className="bg-orange-100 border-2 border-orange-300 rounded-lg p-2 text-center">
                  <p className="text-orange-800 font-bold text-sm">Ã°Å¸â€â€™ Auto-Locked (No Time)</p>
              ...
              {isLocked && actualTimeEarned > 0 && (
                <div className="bg-red-100 border-2 border-red-300 rounded-lg p-2 text-center">
                  <p className="text-red-700 font-bold text-sm">Ã°Å¸â€â€™ Manually Locked</p>
                </div>
              )}
                                  ...
                                  <div key={a.id} className="bg-white rounded-lg p-4 flex items-center justify-between">
                                    <div className="flex-1">
                                      ...
                                      <p className="text-sm text-gray-600">
                                        {a.mins} min reward Ã¢â‚¬Â¢ {a.passScore || a.pass_score}% to pass
                                      </p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                                        ...
                                        {a.status === 'completed' ? `Ã¢Å“â€œ ${a.score}%` : 'Pending'}
                                      </span>
                                      {a.status === 'pending' && (
                                        <button
                                          onClick={() => deleteAssignment(a.id)}
                      ...
                      return (
                        <div key={a.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {child?.name} Ã¢â‚¬Â¢ {a.mins}min reward
                              {isFuture && (
                                <span className="ml-2 text-orange-600 font-semibold">
                                  Ã°Å¸â€œâ€¦ {new Date(availDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                                </span>
                              )}
                            </p>
                          </div>
...
const ChildView = ({ user, onLogout }) => {
  ...
  const calcResults = async (ans, course) => {
    ...
    if (updated) {
      console.log('Ã¢Å“â€¦ Database update successful!', updated);
    } else {
      console.error('Ã¢ÂÅ’ Database update failed - but continuing with state update');
    }
                          ...
                          <div className="space-y-1">
                            {question.pairs.map((pair, pairIdx) => (
                              <p key={pairIdx} className={isCorrect ? 'text-green-700' : 'text-red-700'}>
                                {pair.left} â†’ {question.pairs[userAnswer?.[pairIdx]]?.right || '(Not matched)'}
                              </p>
                            ))}
                          </div>
                        )}
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50">
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className={`px-4 py-2 rounded-lg font-semibold text-sm ${
              ...
              {isLocked ? 'Ã°Å¸â€â€™ Locked' : 'Ã¢Å“â€œ Unlocked'}
            </div>
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="bg-white rounded-3xl shadow-lg p-6">
          ...
          {isLocked && remaining === 0 && (
            <div className="bg-orange-100 border-2 border-orange-300 rounded-xl p-4 mb-4">
              <div className="flex items-center gap-3">
                <div className="text-3xl text-orange-600 font-bold">Ã¢ÂÂ±Ã¯Â¸Â</div>
                <div className="flex-1">
          ...
          {isLocked && remaining > 0 && (
            <div className="bg-red-100 border-2 border-red-300 rounded-xl p-4 mb-4">
              <div className="flex items-center gap-3">
                <div className="text-3xl text-red-600 font-bold">Ã°Å¸â€â€™</div>
                <div className="flex-1">
          ...
          <p className="text-center text-sm text-gray-500 mt-2">
            {timeData.used}m used Ã¢â‚¬Â¢ {timeData.total}m earned
          </p>
          
          <div className="mt-4 bg-purple-50 border border-purple-200 rounded-xl p-3">
            <p className="text-xs text-purple-800 text-center">
              Ã°Å¸â€™Â¡ Tip: Pause your timer when stepping away to save your screen time!
            </p>
          </div>
        </div>
        ...
        {assigns.filter(a => a.status === 'completed').length > 0 && (
          <div className="bg-white rounded-3xl shadow-lg p-6">
            <h2 className="text-xl font-bold mb-4 text-green-600">Ã¢Å“â€œ Completed Courses</h2>
            <div className="space-y-3">
                ...
                return (
                  <div key={a.id} className="border-2 border-green-200 bg-green-50 rounded-2xl p-4">
                    ...
                    <div className="flex gap-3 mt-2 text-sm text-gray-600">
                      <span className="bg-green-100 px-3 py-1 rounded-full">Ã¢Å“â€œ Completed</span>
...
const AdminView = ({ user, onLogout }) => {
  ...
  const deleteUser = async (userId) => {
    ...
    try {
      ...
      if (deleteError) {
        console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Database delete failed:', deleteError);
      ...
      console.error('ÃƒÂ¢Ã‚ÂÃ…â€™ Error deleting user:', err);
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                  ...
                  <span className="text-green-600 font-semibold">Ã¢Å“â€œ Online</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl p-6 shadow-lg">
              <div className="flex justify-between items-center mb-6">
                ...
                {editingCourse && (
                  <button onClick={() => { setEditingCourse(null); setNewCourse({ title: '', category: '', subject: '', description: '', intro: '', summary: '', sections: [{ title: '', text: '', images: [], videos: [] }], questions: [{ q: '', opts: ['', '', '', ''], ans: 0 }] }); }} className="text-red-600 hover:text-red-700">
                    Cancel Edit
                  </button>
                )}
              </div>
                ...
                {courses.map(course => (
                  <div key={course.id} className="flex justify-between items-center p-4 border-2 rounded-xl">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{course.subject} Ã¢â‚¬Â¢ {course.sections.length} sections Ã¢â‚¬Â¢ {course.questions.length} questions</p>
                    </div>
                    <div className="flex gap-2">
                      <button 
                        onClick={() => { 
                      ...
                      >
                        {loadingCourse ? (
                          <>
                            <span className="animate-spin">Ã¢ÂÂ³</span>
                            Loading...
                          </>
                        ) : (
                          'Edit'
